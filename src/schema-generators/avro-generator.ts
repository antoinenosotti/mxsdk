...
function main() {
    ...
    sampleDomain.revision.entities.filter(entity => entity.documentation.indexOf(aggregate) > -1).forEach((entity: any) => {
        let fields = entity.attributes.filter((attribute: any) => attribute.documentation.indexOf("@aggregate-ignore") === -1).map((attribute: any) => {
            const
        ...
        fields = fields.concat(entity.associations.filter((association: any) => (association.parent && (association.documentation.indexOf(aggregate) > -1))).map((association: any) => {
            return {
